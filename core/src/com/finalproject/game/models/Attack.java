...
public enum Attack {
    BACKHANDED_COMPLIMENT("Backhanded Compliment", AttackType.SARCASM, 15,
            "A compliment that's actually an insult. Leaves the target questioning their self-worth."),
    SCOPE_CREEP("Scope Creep", AttackType.ANGULAR, 20,
            "Slowly increases the complexity until the target is overwhelmed."),
    ...
    GROANER("Groaner", AttackType.DAD_JOKE, 8,
            "Makes the target groan and roll their eyes, slightly damaging their morale."),
    ...
    LITERAL_LAUNCH("Literal Launch", AttackType.LITERALISM, 15,
            "Launches the target, because they asked for 'a lift'."),
    OFF_KEY("Off Key", AttackType.KARAOKE, 10, "Blasts a note so off-key, it disrupts the enemy's focus."),

    // Special attack

    POWER_OF_LITANG("Litang Power", AttackType.SPECIAL_RELX, 30,
            "Bring out the power of Litang! Ding's special attack! "),

    PRIVACY_INVASION("Privacy Invasion", AttackType.SPECIAL_ZUCK, 33,
            "Use collected data on ButtBook to invade the enemy's privacy!, Zuck's special attack!"),

    SPACE_WINER("Space Winer", AttackType.SPECIAL_JEZO, 35,
            "Use the Space Origin Rocket that looks like a sausage to hit the enemy!"),
